...
export const GET: RequestHandler = ({ url }) => {
  ...
  const hasEvalConfig = fs.existsSync(path.join(resolved, "eval.config.ts"));
  const hasEvalDir =
    fs.existsSync(path.join(resolved, "eval")) && fs.existsSync(path.join(resolved, "eval", "eval.config.ts"));
